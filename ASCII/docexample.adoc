...
[NOTE]
====
- ðŸ”° fundamental topics for beginners getting started with DevSensei
- ðŸ”¬ advanced topics for once you have your first workflows automated
====
...
=== Syntax support in IDE for DevSensei configuration ðŸ”°
...
== Migration: From Code Owners to DevSensei ðŸ”°
...
=== Code Owners Settings NOT supported in DevSensei add-codeowners action ðŸ”¬
...
a|`conditions` (optional, but probably wanted) ðŸ”° a|
A set of conditions to be met for the actions of the workflow to be executed for a pull request
a|`retrigger-on` (optional) ðŸ”¬ a| A set of values. Causes additional trigger for actions when the value changes while the condition is currently met.
...
==== Aggregate condition operators ðŸ”¬
...
=== Share Configuration with Includes ðŸ”¬
...
==== Sharing Parts of the Configuration with YAML Anchors ðŸ”¬
...
=== Configuration-Changes and Existing Pull Requests ðŸ”¬
